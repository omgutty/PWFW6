/**
 * Interview Answer — Reporter Architecture

"Our framework uses a custom Reporter that implements Playwright's Reporter
 interface giving us full control over how test results are presented. 
 The reporter is wired into playwright.config.ts by file path — 
 Playwright compiles and instantiates it directly, separate from 
 the normal TypeScript import system. We run four reporters in parallel
  — our custom TTA reporter for the branded stakeholder-facing HTML,
   Playwright's built-in HTML for developer debugging with trace and video viewer,
    JSON for CI/CD pipeline integration, and list for real-time console feedback.
     The TTA reporter writes a timestamped HTML file per run and maintains a history index,
      giving teams a complete audit trail without needing an external test management tool.
       During execution the report auto-refreshes every 5 seconds via a meta refresh tag so 
       stakeholders can monitor progress in real time."
 */
/**
...
class CustomTTAReporter implements Reporter {
    ...
    onBegin(config: FullConfig, suite: Suite): void {
        ...
        console.log('\n╔════════════════════════════════════════════════════════════════╗');
        console.log('║        🎭 CYGNET PLAYWRIGHT AUTOMATION - REAL-TIME REPORT         ║');
        console.log('╠════════════════════════════════════════════════════════════════╣');
...
<body>
    <div class="header"><h1>📊 TTA Report History</h1><p>Cygnet - Playwright Framework</p></div>
